...
- Transcript selection keeps the original message text instead of copying rendered soft wraps,
  padding, or transformed Markdown, with a high-contrast selection highlight across themes.
- `Ctrl+C` now copies active selections from the transcript, composer, and single-line fields;
  `Ctrl+V`/`Cmd+V` reads the native system clipboard asynchronously in every editable field.
- Native clipboard helpers are preferred over duplicate OSC 52 writes, and rapid copies are
  coalesced to prevent terminal flicker, UI stalls, and stale writes finishing out of order.
...
- Expanded the hermetic suite to 582 passing tests.
